...
class DeleteAdminDialog extends React.Component {
  ...
  deleteAdmin() {
    ...
    deleteAdminVotechain(account, votechain, adminToBeDeleted.id)
    handleClickCloseDialog()
  }

  render() {
    const { classes, openDialog, handleClickCloseDialog, adminToBeDeleted } = this.props
              ...
              return (
                <Grid item key={field.field} style={{width: '100%'}}> 
                  ...
                  >
                    ...
                    <Grid item xs={9}><Typography className={classes.fieldValue}>{adminToBeDeleted[field.field]}</Typography></Grid>
                  </Grid>
                </Grid>
              )  
            })}
