...
contract("Votechain - data insertion", async(accounts) => {
  ...
  it("should add bulk of voters.", async () => {
    ...
    let expectedStudentNo2 = '2015-09899';
    let expectedName2 = 'Alley';

    let expectedVoterKeyList = [expectedVoterKey, expectedVoterKey2];
    let expectedStudentNoList = [expectedStudentNo, expectedStudentNo2];
    let expectedNameList = [expectedName, expectedName2];

    await votechainInstance.addBulkVoterAt(expectedElectionKey, expectedVoterKeyList, expectedStudentNoList, expectedNameList);

    // verify if the first voter was successfully recorded
    let isVoter = await votechainInstance.isVoter(expectedVoterKey).call();
    expect(isVoter, 'The first voter should be added.').to.be.true;

    // verify if the second voter was successfully recorded
    let isVoter2 = await votechainInstance.isVoter(expectedVoterKey2).call();
